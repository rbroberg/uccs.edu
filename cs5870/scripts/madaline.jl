...
for s in 1:4
	...
	for i in 1:size(X)[1]
		...
		# -------------------------------------------------------------------------
		z_out = (z_in.>T)*1; # calculate z_out, threshold = 0
		...
		elseif y==1
			...
			k = indmin(z_in.^2)
			w[k,1:(end-1)]=w[k,1:(end-1)]+a*(y-z_in[k])*x[k] # z_in wts
			w[k,end]=w[k,end]+a*(y-z_in[k]) # bias wt 
		elseif y==-1
