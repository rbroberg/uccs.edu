...
def load_data(case,npre,ninter,ntest,features, split):
    ...
    #TODO: randomize draw
    x_train=np.vstack((dat_train[0:mpre,:],dat_train[npre:(npre+minter),]))
    x_valid=np.vstack((dat_train[mpre:npre,:],dat_train[(npre+minter):(npre+ninter),]))
    y_train=np.hstack((np.ones(mpre),np.zeros(minter)))
    y_valid=np.hstack((np.ones(npre-mpre),np.zeros(ninter-minter)))
    x_test=dat_test
