    ...
    for N in [8]
		for s in [2,8,32]
			...
			for m in 1:runs
				...
				#println(join([innie, wins, mean(starts), mean(scores), mean(niters), mean(innies)],'\t'))
				#flush(STDOUT)
				push!(Tstarts,mean(starts))
				push!(Tscores,mean(scores))
				push!(Tniters,mean(niters))	
				push!(Tninnie,mean(innies))	
			end
			println(join([N, runs, innerruns, s, Twins, (1.*Twins)/runs, mean(Tstarts), mean(Tscores), mean(Tniters),mean(Tninnie)], '\t'))
			flush(STDOUT)
		end
	end
	#print(B)
